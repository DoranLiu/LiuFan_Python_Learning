'''
MySQL的中定义数据字段的类型对你数据库的优化是非常重要的。

MySQL的支持多种类型，大致可以分为三类：
...
BOOL, BOOLEAN	-	布尔值
BIT [（M）]
二进制位（101001）中，m表示二进制位的长度（1-64），默认m = 1的
...
整数类型中的m仅用于显示，对存储范围无限制。例如：int(5)当插入数据2时，select时数数据显示为：00002
...
准确的小数值，m是数字总数（负号不算），d是小数点后个数。m最大值为65，d最大值为30。

特别的：对于精确数值计算时需要用此类型decaimal能够存储精确值的原因在于其内部按照字符串存储。

FLOAT [（M，D）] [UNSIGNED] [ZEROFILL]
单精度浮点数（非准确小数值）中，m是数字总个数，D是小数点后个数。
...
双精度浮点数（非准确小数值）中，m是数字总个数，D是小数点后个数。
...
BOOL，BOOLEAN
这些类型是TINYINT。的同义词零值被认为是假的非零值被认为是正确的：
...
每个时间类型有一个有效值范围和一个”零”值，当指定不合法的MySQL的不能表示的值时使用”零”值。
...
类型	大小（字节）	用途
CHAR(M)	0 to 255	固定长度的字符串，即使数据小于中号长度，也会占用中号长度
VARCHAR(M)	0 to 65535	一个可变长度的字符串，男表示在字符的最大列长度
...
VARCHAR（M）注
虽然VARCHAR(M)使用起来较为灵活，但是从整个系统的性能角度来说，CHAR(M)数据类型的处理速度更快，甚至有时可以超出VARCHAR(M)处理速度的50％。因此，用户在设计数据库时应当综合考虑各方面的因素，以求达到最佳的平衡
...
COMMIT;
SET('value1','value2',...)
SET是一个字符串对象，可以它有0或更多个值，每个值均必须选自一个允许值列表中，该列表在表创建时被指定。
