# coding: utf-8
'''
Builder Design Pattern 建造者设计模式

我们想要创建一个由多个部分构成的对象，而且它的构成需要一步接一步地完成。 只有当各个部分都创建好，这个对象才算是完整的。此时需要建造者设计模式。

在这一点上，建造者模式和工厂模式的差别并不太明确。主要的区别在于工厂模式以单个步 骤创建对象，而建造者模式以多个步骤创建对象，并且几乎始终会使用一个指挥者。
