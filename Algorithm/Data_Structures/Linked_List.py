...
class ReverseListNode:
    ...
    def reverse(self, head):
        prev = None
        while head: # 在访问下一个点时，要判断当前点是否为None
...
# 双向链表
'''
双向链表和单向链表的区别在于：双向链表的反转核心在于next和prev域的交换，还需要注意的是当前节点和上一个节点的递推。
'''
class DListNode:
    def __init__(self, val):
        self.val = val
...
然后只需要把 prev -> next = prev -> next -> next 即可。但是由于链表表头可能在这个过程中产生变化，导致我们需要一些特别的技巧去处理这种情况。比如 Dummy Node。
...
除此之外，还有一种用法比较少见，就是使用 dummy node 来进行head的删除操作，一般的方法current = current.next 是无法删除 head 元素的，所以这个时候如果有一个dummy node在head的前面。
...
2。判断单链表是否有环 利用快慢指针的原理，同样设置两个指针 *fast、*slow 都指向单链表的头节点，其中 *fast的移动速度是*slow的2倍。
...
class NodeCircle:
    ...
    def has_circle(self,head):
        slow = head
        fast = head
        while (slow and fast): # 没结束，head也不是none
            fast = fast.next
            slow = slow.next

            if fast:
                fast = fast.next # fast每次走两步，这是第二步
